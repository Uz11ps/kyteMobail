# Настройка SMS.ru для отправки SMS

## Быстрая настройка

### 1. Обновить .env файл на сервере

Выполните на сервере следующие команды:

```bash
cd /var/www/kyte-backend/backend

# Добавить настройки SMS.ru в .env
echo "" >> .env
echo "SMS_PROVIDER=smsru" >> .env
echo "SMSRU_API_ID=2BD84383-DDFD-B4E3-A588-5908F91C3927" >> .env

# Проверить что добавилось
cat .env | grep -E "SMS|SMSRU"
```

### 2. Перезапустить PM2

```bash
# Остановить все процессы Node.js
sudo killall node || true
sleep 2

# Перезапустить через PM2
cd /var/www/kyte-backend/backend
pm2 delete kyte-backend || true
pm2 start src/server.js --name kyte-backend --update-env
pm2 save

# Проверить логи
pm2 logs kyte-backend --lines 20 --nostream | grep -E "SMS|📱"
```

### 3. Проверить работу

```bash
# Отправить тестовый запрос
curl -X POST http://94.131.88.135/api/auth/phone/send-code \
  -H 'Content-Type: application/json' \
  -d '{"phone": "+79686288842"}'

# Проверить логи
pm2 logs kyte-backend --lines 30 --nostream | tail -20
```

## Формат номера телефона

SMS.ru автоматически преобразует номер из формата `+79991234567` в `79991234567` (без знака `+`).

Поддерживаются форматы:
- `+79991234567` (с плюсом)
- `79991234567` (без плюса)
- `89991234567` (с 8, будет преобразован в +7)

## Проверка баланса SMS.ru

Для проверки баланса можно использовать:

```bash
curl "https://sms.ru/my/balance?api_id=2BD84383-DDFD-B4E3-A588-5908F91C3927&json=1"
```

## Важные замечания

1. **API ID**: `2BD84383-DDFD-B4E3-A588-5908F91C3927` - это ваш ключ доступа к SMS.ru
2. **Лимиты**: SMS.ru имеет лимиты на количество SMS в день в зависимости от тарифа
3. **Стоимость**: Проверьте стоимость отправки SMS в личном кабинете SMS.ru
4. **Поддержка стран**: SMS.ru работает в России и Казахстане

## Отладка

Если SMS не приходят:

1. Проверьте логи PM2:
```bash
pm2 logs kyte-backend --lines 50 --nostream | grep -E "SMS|📱|📤|❌|✅"
```

2. Проверьте баланс в SMS.ru:
```bash
curl "https://sms.ru/my/balance?api_id=2BD84383-DDFD-B4E3-A588-5908F91C3927&json=1"
```

3. Проверьте статус отправки (замените SMS_ID на реальный ID из логов):
```bash
curl "https://sms.ru/sms/status?api_id=2BD84383-DDFD-B4E3-A588-5908F91C3927&sms_id=SMS_ID&json=1"
```

